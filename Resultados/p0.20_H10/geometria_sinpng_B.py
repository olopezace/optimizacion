...
'''
    ...
    mon1 = sim.add_dft_fields([mp.Bz, mp.Ey], frecuencias,
                            where=mp.Volume(center=mp.Vector3(Nlamda/(2*fminima), (b+d)/2, 0),
  ...
  #  plt.title("Geometría en 'H' del híbrido en cuadratura")
   ...
   # plt.title("Vista 2D del campo Elcectrico en su componente Ey")
    ...
    plt.plot(np.array(freqs), flujofunete, label='Puerto 1: Incidente')
    plt.plot(np.array(freqs), flujos2, label='Puerto salida 2')
    plt.plot(np.array(freqs), flujos3, label='Puerto salida 3 ')
   ...
   # plt.title('Espectro de Potencia en cada puerto')
    ...
    # S-PARÁMETROS
    S12 = 10 * np.log10(flujos2 / flujofunete)
    S13 = 10 * np.log10(flujos3 / flujofunete)

        #Parámetros S
    plt.figure(figsize=(8,6))

    plt.plot(np.array(freqs), S12, label='S21 ', color='tab:orange')
    plt.plot(np.array(freqs), S13, label='S31 ', color='tab:green')
    plt.plot(np.array(freqs), -3*np.ones_like(freqs), 'b--', label='-3 dB')
    plt.plot(np.array(freqs), -3.5*np.ones_like(freqs), 'k--')
    plt.plot(np.array(freqs), -2.5*np.ones_like(freqs), 'k--')
   ...
   # plt.title('Parámetros S del híbrido en H')
    ...
    #deltaA = np.abs(S12/S13)
    #deltaA1 = np.abs(S13/S12)
    deltaA = np.sqrt(flujos2 / flujos3)
    plt.figure(figsize=(10,6))
    plt.plot(freqs, deltaA)
    #plt.plot(freqs, deltaA1, label='Relación de amplitud |S12/S13|')
...
#desbalance de amplitud en dB
    deltaA_dB = np.abs(S12 - S13)

    plt.figure(figsize=(10,6))
    plt.plot(freqs, deltaA_dB, label = '$\Delta{A}$ ')
    plt.plot(freqs, [0]*len(freqs), "k--" )  
    plt.plot(freqs, [1]*len(freqs), "k--" )  
    plt.xlabel('Frecuencia (GHz)')
    plt.ylabel('Desbalance de Amplitud [dB]') 
   # plt.title('Desbalance de Amplitud entre Puertos del Híbrido en H')
    plt.legend()
    plt.grid()
    plt.savefig("Desbalance_Amplitud_dB.png", dpi=300)
    plt.close()     


    # desfase en grados con los campos 
    #ang = np.angle(S12 / S13, deg=True)
    ...
    ang_E = np.angle(Ey2/Ey1, deg=True)
    meanfase = round(np.mean(ang_E),0) 

    plt.figure(figsize=(10,6))
    plt.plot(freqs, ang_E)
    plt.plot(freqs, [90]*len(freqs), "k--" , label=' Desfase ideal = 90°')
    #plt.plot(freqs,[meanfase]*len(freqs), label = f"promedio de la fase {meanfase}")
    plt.xlabel('Frecuencia (GHz)')
    plt.ylabel('Diferencia de Fase [deg]')
    #plt.title('Diferencia de Fase entre Puertos del Híbrido en H')
    plt.legend()
    plt.grid()
    plt.savefig("Desfase_E.png", dpi=300)
    plt.close()


   # desfase en grados con los campos 
    # desfase en grados con los S-Parámetros
    ang_S = abs(np.arctan(S12, S13) * (180/np.pi))
    ang_S2 = abs(np.arctan(S13, S12) * (180/np.pi))
    

    plt.figure(figsize=(10,6))
    plt.plot(freqs, ang_S)
    #plt.plot(freqs, ang_S2, label='Diferencia de fase ∠(S12/S13)')
    plt.plot(freqs, [90]*len(freqs), "k--" , label=' Desfase ideal = 90°')
    plt.xlabel('Frecuencia (GHz)')
    plt.ylabel('Diferencia de Fase [deg]')
    #plt.title('Diferencia de Fase entre Puertos del Híbrido en H')
    plt.legend()
    plt.grid()
    plt.savefig("Desfase_S.png", dpi=300)
    plt.close()
    ...
    parametros = freqs, S12, S13, deltaA
    return parametros
...
#parametros_de_prueba = [2.6398753, 1.4420946, 0.72992139, 0.45434774, 0.80359054]
#parametros_de_prueba = [2.9292654420341555, 1.519597906456931,0.7072093856351066, 0.38601826378854426, 0.5]

#ejecutar_simulacion(parametros_de_prueba)

#solucion1 = [2.711, 1.527, 0.548, 0.386, 0.782]

#solucion2 = [2.934, 1.379, 0.631, 0.321, 1.003]

#solucion3bio01 = [2.75738988, 1.37421849, 0.68043533, 0.29908317, 0.77139861]

solucionbio05= [2.74056048, 1.39140571, 0.63422703, 0.32153898, 0.8680746 ]
#solucionbio05round = [np.round(2.74056048,3), np.round(1.39140571,3),np.round(0.63422703,3), np.round(0.32153898,3),np.round(0.8680746,3)   ]


ejecutar_simulacion(solucionbio05)
